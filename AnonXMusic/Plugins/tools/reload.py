...
@app.on_message(
    filters.command(["admincache", "reload", "refresh","رفرش"]) & filters.group & ~BANNED_USERS
)
@language
...
@app.on_message(filters.command(["reboot","روبوت"]) & filters.group & ~BANNED_USERS)
@AdminActual
...
async def close_menu(_, query: CallbackQuery):
    try:
        ...
        umm = await query.message.reply_text(
            f"تم الإيقاف بواسطة: {query.from_user.mention}"
        )
