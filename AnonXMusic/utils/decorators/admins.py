        ...
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(
                    text=f"{app.mention} تحت صيانة، اذهب الي href={SUPPORT_CHAT}>مجموعة المساعدة</a> لتعرف السبب.",
                ...
                if not admins:
                    ...
                    if message.from_user.id not in admins:
                        if await is_skipmode(message.chat.id):
                            ...
                            text = f"""<b>يجب ان تكون ادمن</b>

يمكن ان تفعل رفرش بهاذا الامر : رفرش

» {upvote} الأصوات اللازمة لأداء هذا الإجراء."""
        ...
        if await is_maintenance() is False:
            if CallbackQuery.from_user.id not in SUDOERS:
                return await CallbackQuery.answer(
                    f"{app.mention} تحت صيانة، اذهب الي href={SUPPORT_CHAT}>مجموعة المساعدة</a> لتعرف السبب.",
                    show_alert=True,
                )
        try:
